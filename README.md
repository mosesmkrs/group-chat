...
## Overview
This is a multithreaded, client-server command-line application written in C. It functions as an online Group Chatting application that allows users to create, join, leave, search for groups and send messages to all members of a group they belong to. 

The application is built specifically for Windows using the **Winsock2** API for networking and Windows native threading (`CreateThread`) for handling concurrent user connections.
...
How to Build
Open your PowerShell or Command Prompt terminal.
...
(To clean the build directory, you can run mingw32-make clean)
